...
public class GameDAO {
	...
	public ArrayList<Game> getGamesThisWeek() {
		...
		try {
			...
			ps = connection.prepareStatement("select season_year, week, season_type " + "from game "
					+ "where date(start_time) <= current_date " + "order by start_time desc");
			rs = ps.executeQuery();
			if (rs.next()) {
	...
	public int[] getYears() {
		...
		try {
			connection = Util.getConnection();
			ps = connection
					.prepareStatement("select season_year from game group by season_year order by season_year desc");
			rs = ps.executeQuery();
			while (rs.next()) {
	...
	public void printYearAndType() {
		Connection connection = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			connection = Util.getConnection();
			ps = connection.prepareStatement("select " + "season_year, season_type, count(*) from game "
					+ "group by season_year, season_type " + "order by season_year desc, season_type");
			rs = ps.executeQuery();
			while (rs.next()) {
	...
	public ArrayList<Game> getGames(int week) {
		...
		try {
			connection = Util.getConnection();
			ps = connection.prepareStatement("select a.*, date(a.start_time) as gamedate,  "
					+ "b.name as home_team_name, c.name as away_team_name, "
					+ "start_time > current_timestamp as pending " + "from game as a " + "join team as b "
					+ "on a.home_team = b.team_id " + "join team as c " + "on a.away_team = c.team_id "
					+ "where week = ? " + "order by season_type, start_time desc");
			ps.setInt(1, week);
			rs = ps.executeQuery();
			boolean done = false;
			int year = 0;
	...
	public ArrayList<Game> getGames(int year, int week, String type) {
		...
		try {
			connection = Util.getConnection();
			String sqlStatement = "select b.name as home_team_name, " + "c.name as away_team_name, "
					+ "a.home_score, a.away_score, a.start_time, a.day_of_week, a.week, "
					+ "a.season_year, a.season_type, a.finished, " + "date(a.start_time) as gamedate,  "
					+ "b.name as home_team_name, c.name as away_team_name, "
					+ "start_time > current_timestamp as pending " + "from game as a " + "join team as b "
					+ "on a.home_team = b.team_id " + "join team as c " + "on a.away_team = c.team_id "
					+ "where a.season_year = ? " + "and a.week = ? " + "and a.season_type::text = ? "
	...
	public ArrayList<Game> getGames() {
		...
		try {
			...
			ps = connection.prepareStatement(
					"select a.*, date(a.start_time) as gamedate, b.name as home_team_name, c.name as away_team_name, "
	...
	public int getCurrentWeek() {
		...
		try {
			connection = Util.getConnection();
			ps = connection.prepareStatement(
					"select * from game where date(start_time) <= current_date order by start_time desc");
			rs = ps.executeQuery();
			if (rs.next()) {
